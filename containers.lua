...
containers_table = {
    ["1, 1-2"] = {name="Amphora",                            cost=60, weight=40, soft=false, capacity=60, sealed=true},
    ["1, 3-4"] = {name="Barrel",                             cost=65, weight=20, soft=false, capacity=50, sealed=true},
    ["1, 5-6"] = {name="Basket",                             cost=1.75, weight=1.5, soft=true, capacity=60, sealed=false},
    ["2, 1-2"] = {name="Bottle/Jar",                         cost=3, weight=1, soft=false, capacity=40/16, sealed=true},
    ["2, 3-4"] = {name="Bottle/Jar, Small",                  cost=2, weight=0.5, soft=false, capacity=20/16, sealed=true},
    ...
    ["4, 4"]   = {name="Canteen, Wood, 1 quart",             cost=10, weight=3, soft=false, capacity=40/16, sealed=true},
    ...
    ["6, 2"]   = {name="Vial, Crystal",                      cost=5, weight=0.25, soft=false, capacity=10/16, sealed=true},
    ["6, 3"]   = {name="Vial, Iron",                         cost=15, weight=0.5, soft=false, capacity=10/16, sealed=true},
    ["6, 4"]   = {name="Wineskin, 1 Gallon",                 cost=10, weight=0.25, soft=true, capacity=10, sealed=true},
...
containers_by_most_efficient_table = {}
for _,v in pairs(containers_table) do
    if v.capacity then
        table.insert(containers_by_most_efficient_table, v)
    end
end
table.sort(containers_by_most_efficient_table, function(a,b)
    if a.capacity / a.weight == b.capacity / b.weight then
        return a.cost < b.cost
    else
...
function get_best_container(is_fluid)
    local best_container = nil
    for _,v in ipairs(containers_by_most_efficient_table) do
        if v.sealed or not is_fluid then
            best_container = v
        end
    end
    if best_container == nil then
